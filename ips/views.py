# las vistaspueden heredar de diferentes clases
import logging
...
def CreatePatient(request):
    ...
    if request.method == 'POST':
        ...
        if patient_form.is_valid():
            patient_form.save()
            return redirect('ips:patients')
    else:
        patient_form = PatientForm()
    tipos_documento = DocumentType.objects.all()
...
class ContactList(ListView):
    ...
    context_object_name = 'contacts'  # nombre del objeto que le voy a dar para ser enviado al template
    queryset = Contact_service.objects.filter(uuid__isnull=True)  # Siempre va a ser esta consulta por defecto a no ser que se cambie


class CreateContact(CreateView):
    model = Contact_service
    form_class = Contact_serviceForm
    template_name = 'contact/create.html'
    success_url = reverse_lazy('ips:contacts')  # Al finalizar la petición redireccionar
