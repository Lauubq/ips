...
class Occupation(models.Model):
    code = models.CharField(verbose_name="Código", max_length=3, primary_key=True)
    name = models.CharField(verbose_name="Descripción", max_length=50, unique=True, null=False)
    father = models.ForeignKey("self", verbose_name = "Padre", on_delete = models.PROTECT) ##models.CASCADE
    class Meta:
        db_table = 'occupation'
...
class Wills_document(models.Model):
    document = models.CharField(verbose_name="Documento", max_length=3, primary_key=True)
    provider_code = models.CharField(verbose_name="Codigo de prov", max_length=50, unique=True, null=False)
    date = models.DateTimeField(verbose_name="Fecha", max_length=50, unique=True, null=False )
    class Meta:
    ...
    def __str__(self):
        return f'{self.document} - {self.provider_code} - {self.date}'
...
class Patient(models.Model):
    ...
    wills_document = models.ForeignKey(Wills_document, verbose_name = "Documento de ", on_delete = models.PROTECT)
...
class Contact_service(models.Model):
    uuid = models.UUIDField(primary_key = True, default = uuid.uuid4, editable = False)
    date = models.DateTimeField('Fecha', auto_now = True, auto_now_add = False)
